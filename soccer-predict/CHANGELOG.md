...
对齐「代码级修复」的两根因，消除 skill 文档中同源的逻辑缺陷：

- **规则69 跨联赛排名禁止直接比较**：比赛双方来自不同联赛（欧冠/欧联/解放者杯/世俱杯，或杯赛淘汰赛跨联赛相遇）时，基本面「联赛排名/积分榜位置/积分差」**不得跨联赛比绝对名次**（「挪超第1 > 德甲第6 → 偏博德闪耀」属尺度错位）。改用：①首选盘口/欧赔隐含强度；②次选「联赛内排名百分比 × 联赛强度系数」归一；③跨联赛遭遇战中该项基本面权重按 0 处理，不进模型。附联赛强度系数参考档。
- **规则70 代表性盘口线选取**：EV/隐含概率引用的盘口线须**刻意选定**（优先澳\*/Crow\* 二选一，二者皆缺取即时平均值），**严禁因某公司在 odds 表「排第一」而默认选它作基准盘**（对齐代码侧已修复的「澳\* 不再因排第一自动成基准盘」）。
- `data-collection.md` 的「基准公司」与「League Standings」两节同步加 ⚠️ 交叉引用。
